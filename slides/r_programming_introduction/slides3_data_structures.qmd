...
You can extract elements from a data frame with two-dimensional indexing: 
`df[rows, columns]`. 

Entire columns can be pulled by name using `df[['col']]` or `df$col`. 

To access specific elements within a column, index the column directly, for example `df$col[3]`.
